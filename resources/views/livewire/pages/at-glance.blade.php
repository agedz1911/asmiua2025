<div>
    ...
    <div class="px-5 md:px-10 pt-0 mt-10 flex flex-wrap gap-2 justify-end md:justify-center items-center">
        <label class="input input-lg input-success rounded-lg w-full max-w-6xl">
            ...
            <input wire:model.live.debounce.750ms="search" type="search" class="grow" placeholder="Search Session Title or Room" />
            @if ($search !== '')
        ...
        <a target="_blank" href="assets/download/schedule-asmiua2026.pdf" class="btn btn-success rounded-lg"><i class="fa fa-download"></i> Download PDF Schedule</a>
    </div>
        ...
        <div class="flex items-center justify-center">
            <div x-data="{ openTab: 0 }" class="lg:w-11/12 w-full mx-auto">
                <div class="mb-4 flex flex-wrap gap-2 p-2 bg-white rounded-lg shadow-md">
                    @foreach ($days as $day)
                        <button x-on:click="openTab = {{ $loop->index }}" :class="{ 'bg-warning text-white': openTab === {{ $loop->index }} }"
                                            ...
                                            <td class="align-top w-40">
                                                @foreach ($day['sessionsByRoom']->get($room, collect()) as $session)
                                                    <button class="hover:shadow-md block w-full my-1 hover:cursor-pointer"
                                                        ...
                                                        <div class="px-0 py-4 w-full rounded-lg bg-green-50 hover:bg-green-100">
                                                            <div class="badge badge-sm badge-accent mb-2">{{ $session->category_sesi }}</div>
                                                            <br>
                                                            {{ $session->time }} <br>
                                                            {{ $session->title_ses }} <br>
                                                            <span class="text-xs text-gray-600">{{ $session->room }}</span>
                                                        </div>
                                                    </button>
                                                        ...
                                                        <div class="modal-box w-11/12 max-w-5xl text-start">
                                                            <div class="flex justify-between mb-3 text-gray-700">
                                                                <div class="text-start">
                                                                    <h3 class="mb-0">Date: <span>{{ \Carbon\Carbon::parse($session->date)->format('d F Y') }}</span></h3>
                                                                    <h3 class="mb-0">Time: <span>{{ $session->time }}</span></h3>
                                                                </div>
                                                                <p>Room: <span>{{ $session->room }}</span></p>
                                                            </div>
                                                            <p class="mb-0 text-gray-700">Session: <span>{{ $session->title_ses }}</span></p>
                                                            <p class="text-gray-700">Moderator: <span>{{ $session->moderator }}</span></p>
